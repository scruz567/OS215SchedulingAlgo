...
def ljf_scheduler(processes):
    ...
    while len(completed_processes) < len(processes):
        ...
        if current_process:
            if ready_queue:
                max_process = max(ready_queue, key=lambda obj: obj.burst_time)
                if max_process.burst_time > current_process.burst_time:
                    ready_queue.append(current_process)
                    time += max_process.burst_time
                    completion_times[processes.index(max_process)] = time
                    waiting_time[processes.index(max_process)] += time - max_process.arrival_time - max_process.burst_time
                    completed_processes.append(max_process)
...
def main():
    ...
    completion_time, turnaround_time, avg_turnaround_time, waiting_time, avg_waiting_time, schedule_length, throughput = ljf_scheduler(processes)
