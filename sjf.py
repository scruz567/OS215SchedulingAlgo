...
def sjf_scheduler(processes):
    ...
    while len(completed_processes) < len(processes):
        ...
        if current_process:
            if ready_queue:
                min_process = min(ready_queue, key=lambda obj: obj.burst_time)
                if min_process.burst_time < current_process.burst_time:
                    ready_queue.append(current_process)
...
def main():
    ...
    completion_time, turnaround_time, avg_turnaround_time, waiting_time, avg_waiting_time, schedule_length, throughput = sjf_scheduler(processes)
